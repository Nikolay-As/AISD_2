...
public:
	...
	void tableOutput() // output the table of frequences and codes
	{
		if (head)
		{
			...
			cout << "| Symbol | Frequency |  Сode |" << endl;
			cout << "|--------|-----------|-------|" << endl;
			cur = cur->next;
			if (cur->code.length() == 2) {
				//cur = cur->next;
				cout << "|    " << cur->symbol << "   |     " << cur->frequency << "     | " << cur->code << "    |" << endl;
			}
			if (cur->code.length() == 3) {
				//cur = cur->next;
				cout << "|    " << cur->symbol << "   |     " << cur->frequency << "     | " << cur->code << "   |" << endl;
			}
			if (cur->code.length() == 4) {
				//cur = cur->next;
				cout << "|    " << cur->symbol << "   |     " << cur->frequency << "     | " << cur->code << "  |" << endl;
			}
			if (cur->code.length() == 5) {
				//cur = cur->next;
				cout << "|    " << cur->symbol << "   |     " << cur->frequency << "     | " << cur->code << " |" << endl;
			}
			...
			cout << "|________|___________|_______|" << endl;
			cout << endl;
		}
	}
